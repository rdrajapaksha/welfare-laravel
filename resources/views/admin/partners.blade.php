...
<form method="POST" action="{{ route('admin.partners.store') }}" enctype="multipart/form-data" class="card-surface mt-6 max-w-xl space-y-3 p-5">
    @csrf
    ...
    @include('admin.partials.image-field', ['name' => 'logo', 'required' => true, 'label' => $d['forms']['photo']])
    <input class="field" name="website" placeholder="https://">
    <button class="btn btn-brand" type="submit">{{ $d['common']['save'] }}</button>
</form>
<div class="mt-8 space-y-4">
    @foreach ($partners as $partner)
        <form method="POST" action="{{ route('admin.partners.update', $partner) }}" enctype="multipart/form-data" class="card-surface space-y-3 p-5">
            @csrf
            @method('PUT')
            ...
            @include('admin.partials.image-field', ['name' => 'logo', 'current' => $partner->logo_url])
