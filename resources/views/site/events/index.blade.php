...
<section class="section-y">
    <div class="container-page">
        <div class="mb-8 flex gap-2">
            <a href="{{ locale_url('/events') }}" class="rounded-full px-3 py-1 text-sm font-bold {{ ! $past ? 'bg-brand-600 text-white' : 'border' }}">{{ $d['events']['upcoming'] }}</a>
            <a href="{{ locale_url('/events', ['filter' => 'past']) }}" class="rounded-full px-3 py-1 text-sm font-bold {{ $past ? 'bg-brand-600 text-white' : 'border' }}">{{ $d['events']['past'] }}</a>
        </div>
        <div class="grid gap-5 md:grid-cols-2">
            @forelse ($events as $event)
                <a href="{{ route('events.show', $event) }}" class="card-surface p-5">
