...
<form method="POST" action="{{ route('admin.programmes.store') }}" class="card-surface mt-6 max-w-xl space-y-3 p-5">
    @csrf
...
<div class="mt-8 space-y-4">
    @foreach ($programmes as $programme)
        <form method="POST" action="{{ route('admin.programmes.update', $programme) }}" class="card-surface space-y-3 p-5">
            @csrf
            @method('PUT')
