<form method="POST" action="{{ route('admin.committee.store') }}" class="card-surface mt-4 grid max-w-4xl gap-3 p-5 sm:grid-cols-2">
    @csrf
...
<div class="mt-6 space-y-4">
    @forelse ($members as $member)
        <form method="POST" action="{{ route('admin.committee.update', $member) }}" class="card-surface grid gap-3 p-5 sm:grid-cols-2">
            @csrf
            @method('PUT')
            <input type="hidden" name="board" value="{{ $member->board->value }}">
            <input class="field" name="name" required value="{{ $member->name }}">
            <input class="field" name="position_en" required value="{{ $member->position_en }}">
            <input class="field" name="position_si" value="{{ $member->position_si }}">
            <input class="field" name="position_ta" value="{{ $member->position_ta }}">
            <input class="field" name="phone" value="{{ $member->phone }}">
            <input class="field" type="number" name="term_from" required value="{{ $member->term_from }}">
            <input class="field" type="number" name="term_to" value="{{ $member->term_to }}">
            <textarea class="field sm:col-span-2" name="bio_en" rows="2">{{ $member->bio_en }}</textarea>
            <textarea class="field" name="bio_si" rows="2">{{ $member->bio_si }}</textarea>
            <textarea class="field" name="bio_ta" rows="2">{{ $member->bio_ta }}</textarea>
